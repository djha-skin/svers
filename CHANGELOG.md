...
- Maven version comparison with qualifier support (`maven-vercmp`, `maven-normalize`)
- RPM version comparison (`rpm-vercmp`, `rpm-normalize`)
- SemVer 2.0 version comparison with build metadata ignoring (`semver-vercmp`, `semver-normalize`)
- RubyGem version comparison (`rubygem-vercmp`, `rubygem-normalize`)
- Python PEP 440 version comparison with local version parts (`python-vercmp`, `python-normalize`)
